...
| Step | Åtgärd                                             | Providers Unlocked                                                 |
| ---- | -------------------------------------------------- | ------------------------------------------------------------------ |
...
|                    | DeepSeek V3.2              | $0.27/$1.10 per 1M       | Inga             | Best price/quality reasoning      |
|                    | xAI Grok-4 Fast            | **$0.20/$0.50 per 1M** 🆕 | Inga             | Fastest + tool calling, ultralow  |
|                    | xAI Grok-4 (standard)      | $0.20/$1.50 per 1M 🆕     | Inga             | Reasoning flagship from xAI       |
|                    | Mistral                    | Free trial + paid        | Begränsat pris   | European AI                       |
|                    | OpenRouter                 | Pay-per-use              | Inga             | 100+ models aggr.                 |
...
|                    | Kimi K2.5 (Moonshot API) 🆕 | Pay-per-use              | Inga             | Direct Moonshot API access        |
...
|                    | Scaleway AI 🆕              | **$0** (1M tokens total) | Begränsat pris   | EU/GDPR, Qwen3 235B, Llama 70B    |
...
| Tier       | Daglig gräns | Prisgräns   | Anteckningar                                           |
| ---------- | ------------ | ----------- | ------------------------------------------------------ |
...
| Tier   | Daglig gräns      | Prisgräns        | Anteckningar                                |
| ------ | ----------------- | ---------------- | ------------------------------------------- |
| Gratis | **1M tokens/day** | 60K TPM / 30 RPM | World's fastest LLM inference; resets daily |
...
| Tier   | Daglig gräns  | Prisgräns        | Anteckningar                              |
| ------ | ------------- | ---------------- | ----------------------------------------- |
| Gratis | **14.4K RPD** | 30 RPM per model | No credit card; 429 on limit, not charged |
...
| Model                         | Prefix | Daglig Fria Kredit | Anteckningar                                |
| ----------------------------- | ------ | ------------------ | ------------------------------------------- |
| `LongCat-Flash-Lite`          | `lc/`  | **50M tokener** 💥  | Största fria kredittid som någonsin funnits |
| `LongCat-Flash-Chat`          | `lc/`  | 500K tokener       | Mångsidig samtal                            |
| `LongCat-Flash-Thinking`      | `lc/`  | 500K tokener       | Argumentation / CoT                         |
| `LongCat-Flash-Thinking-2601` | `lc/`  | 500K tokener       | Jan 2026 version                            |
| `LongCat-Flash-Omni-2603`     | `lc/`  | 500K tokener       | Multimodal                                  |

> 100% gratis medan i offentlig beta. Anmäl dig på
> [longcat.chat](https://longcat.chat) med epost eller telefon. Kredit resets
> varje dag kl 00:00 UTC.

### 🟢 POLLINATIONS AI (Inga API-nycklar krävs) 🆕

| Model      | Prefix | Prisgräns | Provider Behind    |
| ---------- | ------ | --------- | ------------------ |
...
| Tier   | Daily Neurons | Equivalent Usage                        | Anteckningar            |
| ------ | ------------- | --------------------------------------- | ----------------------- |
| Gratis | **10,000**    | ~150 LLM resp / 500s audio / 15K embeds | Global edge, 50+ models |
...
| Tier   | Free Quota    | Location     | Anteckningar                        |
| ------ | ------------- | ------------ | ----------------------------------- |
| Gratis | **1M tokens** | 🇫🇷 Paris, EU | No credit card needed within limits |
...
| Provider         | Free Credits           | Best Model                                   | Prisgräns                    |
| ---------------- | ---------------------- | -------------------------------------------- | ---------------------------- |
...
| anpassad   | Custom JS function returns true/false            | `(output) => output.length > 10` |
...
- 🔗 **OpenCode Integration** — Native provider support for the OpenCode AI
  coding IDE
- 🔗 **INTEGRERING AV TRAE** — Full support for the TRAE AI development framework
- 📦 **Lagrings-API** — Asynchroniserat lagring för bulkbegäranden
- 🎯 **ruta Baserat på Etiketter** — Rutter anrop baserade på anpassade etiketter
  och metadata
- 💰 **LÃ¥gkostnadskonstellation** — Uppdatera automatiskligen till den
  billigaste tillgängliga leverantören

> 📝 Fullständigt specifikation av funktioner fÃnns i [`dokument / nya-funktioner
> / `](docs/new-features/) (217 detaljerade specifikationer)

---

## 👥 Medarbetare

[![Medarbetare](https://contrib.rocks/image?repo=diegosouzapw/OmniRoute&max=100&columns=20&anon=1)](https://github.com/diegosouzapw/OmniRoute/graphs/contributors)

### Hur man bidrar

1. Forka repositoryn
2. Skapa ditt funktionella gren (`git kÃ¶r upp -b funktion / fantastisk-
   fÃnstÃrf )
3. Spara dina Ã¤ndringar (`git commit -m 'LÃ¤gg till fantastisk funktion'`)
4. Spara till grenen (`git spara original feature/amazing-feature`)
5. Ãppna en pull-begÃ¤ran

Se [CONTRIBUTING.md](CONTRIBUTING.md) fÃrfÃr detaljgÃrningar.

### Utgivning av en Ny Version

```bash
# Utgiv en utgivning — npm utgivs automatiskt
gh utgivare skapa v2.0.0 –title "v2.0.0" –generate-notes
```
...
## Stjärnorna över tiden

## [![Stjärnorna över tiden](https://starchart.cc/diegosouzapw/OmniRoute.svg?variant=adaptive)](https://starchart.cc/diegosouzapw/OmniRoute)
