...
| Imagem                   | Tag      | Tamanho | Description           |
| ------------------------ | -------- | ------- | --------------------- |
...
| Tier               | Provedor                   | Cost                     | Quota Reset      | Best For                          |
| ------------------ | -------------------------- | ------------------------ | ---------------- | --------------------------------- |
| **💳 SUBSCRIPTION** | Claude Code (Pro)          | $20/mo                   | 5h + weekly      | Already subscribed                |
|                    | Codex (Plus/Pro)           | $20-200/mo               | 5h + weekly      | OpenAI users                      |
|                    | Gemini CLI                 | **FREE**                 | 180K/mo + 1K/day | Everyone!                         |
|                    | Copiloto GitHub            | $10-19/mo                | Monthly          | GitHub users                      |
...
|                    | DeepSeek V3.2              | $0.27/$1.10 per 1M       | Nenhum           | Best price/quality reasoning      |
|                    | xAI Grok-4 Fast            | **$0.20/$0.50 per 1M** 🆕 | Nenhum           | Fastest + tool calling, ultralow  |
|                    | xAI Grok-4 (standard)      | $0.20/$1.50 per 1M 🆕     | Nenhum           | Reasoning flagship from xAI       |
|                    | Mistral                    | Free trial + paid        | Taxa limitada    | European AI                       |
|                    | OpenRouter                 | Pay-per-use              | Nenhum           | 100+ models aggr.                 |
...
|                    | Kimi K2.5 (Moonshot API) 🆕 | Pay-per-use              | Nenhum           | Direct Moonshot API access        |
...
|                    | Scaleway AI 🆕              | **$0** (1M tokens total) | Taxa limitada    | EU/GDPR, Qwen3 235B, Llama 70B    |
...
| Modelo              | Prefixo | limit         | Limite de taxa        |
| ------------------- | ------- | ------------- | --------------------- |
...
| Tier       | Limite Diário | Limite de taxa | Notas                                                  |
| ---------- | ------------- | -------------- | ------------------------------------------------------ |
...
| Tier   | Limite Diário     | Limite de taxa   | Notas                                       |
| ------ | ----------------- | ---------------- | ------------------------------------------- |
| Grátis | **1M tokens/day** | 60K TPM / 30 RPM | World's fastest LLM inference; resets daily |
...
| Tier   | Limite Diário | Limite de taxa   | Notas                                     |
| ------ | ------------- | ---------------- | ----------------------------------------- |
| Grátis | **14.4K RPD** | 30 RPM per model | No credit card; 429 on limit, not charged |
...
| Modelo                        | Prefixo | Daily Free Quota | Notas                   |
| ----------------------------- | ------- | ---------------- | ----------------------- |
...
| Modelo     | Prefixo | Limite de taxa | Provider Behind    |
| ---------- | ------- | -------------- | ------------------ |
...
| Tier   | Daily Neurons | Equivalent Usage                        | Notas                   |
| ------ | ------------- | --------------------------------------- | ----------------------- |
| Grátis | **10,000**    | ~150 LLM resp / 500s audio / 15K embeds | Global edge, 50+ models |
...
| Tier   | Free Quota    | Location     | Notas                               |
| ------ | ------------- | ------------ | ----------------------------------- |
| Grátis | **1M tokens** | 🇫🇷 Paris, EU | No credit card needed within limits |
...
| 📂 **Custom Data Directory**  | `DIRETÓRIO_DE_DADOS` override for storage location       |

### Ferramenta Deep Dive

#### Rodapé inteligente com controle de custo prático
...
Quando a quota, taxa ou saúde falhar, o OmniRoute automaticamente muda para o
próximo candidato sem necessidade de alteração manual.

#### Gerenciamento de protocolo que é visível e operável

- MCP + A2A são descobertos na interface e nos documentos (não escondidos)
- APIs do status do protocolo expõem dados operacionais em tempo real
  (`/api/mcp/*`, `/api/a2a/*`)
- Painéis solicitam ações para o dia 2 (combos de alternância, reset de breaker,
  cancelamento de tarefa)

#### Fluxo de trabalho de tradutor + validação

A área de tradução inclui:

- **Território de Jogo**: verifica a transformação de solicitações
- **Testador de Chat**: viagem de solicitação/resposta completa
- **Bancada de Testes**: várias casos em uma execução
- **Monitor Live**: exibição de tráfego em tempo real

Agora validação de protocolo com clientes reais via ` npm run
test:protocols:e2e`.

> 📖 **[READMEA DO SERVIDOR MCP](open-sse/mcp-server/README.md)** — Ferramentas
> de referência, configs do IDE e exemplos de cliente
> 
> 📖 **[READMEA DO SERVIDOR A2A](src/lib/a2a/README.md)** — Habilidades, métodos
> JSON-RPC, transmissão e ciclo de vida de tarefa

## 🧪 Aulas (Evals)

O OmniRoute inclui um framework de avaliação embutido para testar a qualidade da
resposta LLM contra um conjunto ouro de dados. Acesse-o via **Análise → Evals**
no painel.
...
| personalizado | Custom JS function returns true/false            | `(output) => output.length > 10` |
...
| Category                     | Funcionalidades Planejadas | Highlights                                                                             |
| ---------------------------- | -------------------------- | -------------------------------------------------------------------------------------- |
