...
| 🎨 **Provider Icons**              | 130+ provider logos តាងតំណាងតាម `@lobehub/icons` (SVG) កាត់ទឹកដី PNG → generic fallback chain                                                                                      |
| 🔄 ការរៀបចំ **Model Auto-Sync**    | ប្រព័ន្ធផ្សាយប្រទេស 24h និងធនធានសម្ភារីបែបបង្កើតទីកន្លែងរៀបចំរបស់វិធី Sync model lists សម្រាប់ ClosedAI- អ្នកផ្សាយផ្ទាល់                                                           |
| 🌐 អគ្គនាយក **OpenCode Zen/Go**    | សារដើមកើ [X85X]OpenCodeExecutor[X114X] និងអ្នកផ្សាយប្រទេសថ្មីចំនួន 2 ពី @kang-heewon via PR #530: អគ្គនាយកសេវាកម្ម សេវាកម្មអាម៉ាត់ + អាម៉ាត់បមាយ តាម [X85X]OpencodeExecutor[X114X] |
| 🐛 កំណាយ **Gemini CLI OAuth**      | កំណត់ហេតុ [X89X]Actionable error when[X46X] GEMINI_OAUTH_CLIENT_SECRET[X55X] ខ្ជាក់ [X12X] ក្នុង Docker (ទាមទារលើ Google error ផ្សាយ)                                              |
| 🐛 កំណាយ **OpenCode config**       | [X0ពាក្យបម្រុង `saveOpenCodeConfig()` សរសេរ TOML ដូចស្នេហសម្ព័ន្ធ XDG_CONFIG_HOME` ចុង។                                                                                            |
| 🐛 កំណាយ **Pinned model override** | [X0ពាក្យបម្រុង `body.model` បានដាក់ដោយ `pinnedModel` ដើម្បីសញ្ញាបាណ្ឌិក។ខ្លី                                                                                                       |
| 🐛 កំណាយ **Codex/Claude loop**     | [X0ពាក្យបម្រុង `tool_result` បានបញ្ចូលចូលទៅក្នុងទំព័រន់ស្ស៊រ ដើម្បីសញ្ញាបាណ្ឌិក។                                                                                                   |
| 🐛 កំណាយ **Login redirect**        | [X5ការណែនាំ [X5អឹវ ផ្សាយ login អន្តរាយបន្ទាប់ពីការនៅគិតមិនផ្សាយចំណាប់អារម្មណ៍                                                                                                      |
| 🐛 កំណាយ **Windows paths**         | ប្រព័នតំណាឲ្យ Linux ក្រោម MSYS2 / Git- ប្រមាណទៅ បានហាមឃាត់ [X25X]Windows paths[X44X] ([X26X]/c/...[X34X]) ផ្សាយពាក្យ path ដូចជា `C:\...[X61X] បានបណ្តាក់ដ រាប់។                    |

---

## 🖼️ (dashboard) លាភផ្ទាញ
...
| Step | Aksyon                                             | Providers Unlocked                                                 |
| ---- | -------------------------------------------------- | ------------------------------------------------------------------ |
...
|                    | DeepSeek V3.2              | $0.27/$1.10 per 1M       | wala              | Best price/quality reasoning      |
|                    | xAI Grok-4 Fast            | **$0.20/$0.50 per 1M** 🆕 | wala              | Fastest + tool calling, ultralow  |
|                    | xAI Grok-4 (standard)      | $0.20/$1.50 per 1M 🆕     | wala              | Reasoning flagship from xAI       |
|                    | Mistral                    | Free trial + paid        | Limitado ang rate | European AI                       |
|                    | OpenRouter                 | Pay-per-use              | wala              | 100+ models aggr.                 |
...
|                    | Kimi K2.5 (Moonshot API) 🆕 | Pay-per-use              | wala              | Direct Moonshot API access        |
...
|                    | Scaleway AI 🆕              | **$0** (1M tokens total) | Limitado ang rate | EU/GDPR, Qwen3 235B, Llama 70B    |
...
| Model               | Prefix | Limit         | Hangganan ng Rate     |
| ------------------- | ------ | ------------- | --------------------- |
...
| Tier  | Pang-araw-araw na Limitasyon | Hangganan ng Rate | Mga Tala                                    |
| ----- | ---------------------------- | ----------------- | ------------------------------------------- |
| Libre | **1M tokens/day**            | 60K TPM / 30 RPM  | World's fastest LLM inference; resets daily |
...
| Tier  | Pang-araw-araw na Limitasyon | Hangganan ng Rate | Mga Tala                                  |
| ----- | ---------------------------- | ----------------- | ----------------------------------------- |
| Libre | **14.4K RPD**                | 30 RPM per model  | No credit card; 429 on limit, not charged |
...
| Model                         | Prefix | Daily Free Quota | Mga Tala                |
| ----------------------------- | ------ | ---------------- | ----------------------- |
...
| Model      | Prefix | Hangganan ng Rate | Provider Behind    |
| ---------- | ------ | ----------------- | ------------------ |
...
| Tier  | Daily Neurons | Equivalent Usage                        | Mga Tala                |
| ----- | ------------- | --------------------------------------- | ----------------------- |
| Libre | **10,000**    | ~150 LLM resp / 500s audio / 15K embeds | Global edge, 50+ models |
...
| Tier  | Free Quota    | Location     | Mga Tala                            |
| ----- | ------------- | ------------ | ----------------------------------- |
| Libre | **1M tokens** | 🇫🇷 Paris, EU | No credit card needed within limits |
...
| Provider         | Free Credits           | Best Model                                   | Hangganan ng Rate            |
| ---------------- | ---------------------- | -------------------------------------------- | ---------------------------- |
...
| Transkripsyon ng Audio  | `/v1/audio/transcriptions` — 7 providers (Deepgram Nova 3, AssemblyAI, Groq Whisper, HuggingFace, ElevenLabs, OpenAI, Azure), auto-language detection, MP4/MP3/WAV support |
...
### Mga Istratehiya sa Pagsusuri

| Diskarte   | Description                                      | Example                          |
| ---------- | ------------------------------------------------ | -------------------------------- |
...
| kaugalian  | Custom JS function returns true/false            | `(output) => output.length > 10` |
