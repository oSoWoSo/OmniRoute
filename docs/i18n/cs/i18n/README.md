...
když platí, kvóta má strop — 5h používání, týdenní limity nebo minutové limity.
Uprostřed kódovaní poskytovatel přestane reagovat a vývojář ztratí rytmus a
produktivitu.
...
## 🆓 Start zdarma — Konfigurace s Nulovou Cenou
...
Dashboard support for Docker deployments now includes a one-click **Cloudflare
Quick Tunnel** on `Dashboard → Endpoints`. The first enable downloads
`cloudflared` only when needed, starts a temporary tunnel to your current `/v1`
endpoint, and shows the generated `https://*.trycloudflare.com/v1` URL directly
below your normal public URL.

Poznámky:

- Quick Tunnel URLs are temporary and change after every restart.
- Managed install currently supports Linux, macOS, and Windows on `x64` /
  `arm64`.
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want OmniRoute to
  use an existing binary instead of downloading one.

**Using Docker Compose with Caddy (HTTPS Auto-TLS):**

OmniRoute can be securely exposed using Caddy's automatic SSL provisioning.
...
| Tier            | Poskytovatel               | Náklady                                         | Převrat Resetování | Nejlepší pro                                  |
| --------------- | -------------------------- | ----------------------------------------------- | ------------------ | --------------------------------------------- |
| **💳 PŘÍSPĚVEK** | Claude Code (Pro)          | $20/mo                                          | 5h + weekly        | Žádost už jste možná                          |
|                 | Codex (Plus/Pro)           | $20-200/mo                                      | 5h + weekly        | Použivatelé OpenAI                            |
...
|                 | Cerebras                   | **BESPLATNÝ** (1M tokenů/den)                   | 60K TPM / 30 RPM   | Nejrychlejší na světě                         |
|                 | Groq                       | **BESPLATNÝ** (30 RPM)                          | 14,4 tis. RPD      | Ultra-rychlá Llama/Gemma                      |
|                 | DeepSeek V3.2              | $0,27/$1,10 za 1M                               | Nic                | Nejsnadnější poměr cena/výkon                 |
|                 | xAI Grok-4 Fast            | **$0,20/$0,50 za 1M**  🆕                        | Nic                | Nejrychlejší + volání nástrojů, ultra nízká   |
|                 | xAI Grok-4 (standard)      | 0,20 $/1,50 $ za 1M 🆕                           | Nic                | Důvodem je vlajkové modelu ze společnosti xAI |
|                 | Mistral                    | Krátka bezplatná zkušební verze a placená verze | Omezeno            | Evropský AI                                   |
|                 | OpenRouter                 | Pay-per-use                                     | Nic                | Sběr 100+ modelů                              |
| **💰 **          | GLM-5 (pomocí Z.AI) 🆕      | 0,5/1M$                                         | Daily 10AM         | Nová vlajková značka 128K výstupu             |
|                 | GLM-4.7                    | 0,6/1M$                                         | Daily 10AM         | Rozpočet při náhradní operační síle           |
|                 | MiniMax M2.5 🆕             | 0,3/1M vstupy                                   | 5-hour rolling     | Racionalizace + agenty                        |
|                 | MiniMax M2.1               | 0,02$/1M                                        | 5-hour rolling     | Nejnižší varianta                             |
|                 | Kimi K2.5 (Moonshot API) 🆕 | Pay-per-use                                     | Nic                | Prím Herní API přístup                        |
|                 | Kimi K2                    | 9$ za měsíc                                     | 10M vět za měsíc   | Předpřítomné náklady                          |
| **🆓 ZAČÍNEjte** | iFlow                      | **$0**                                          | Unlimited          | 5 modelů neomezeno                            |
|                 | Qwen                       | **$0**                                          | Unlimited          | 4 modelů neomezeno                            |
...
| -------------------- | -------- | ------------- | ---------------------- |
| `Claudiův sonet 4.5` | `kr/`    | **Unlimited** | No reported daily cap  |
| `Claudiův haiku 4.5` | `kr/`    | **Unlimited** | No reported daily cap  |
| `Claudiův opus 4.6`  | `kr/`    | **Unlimited** | Poslední Opus via Kiro |

### 🟢 NÁSTROPNI MODELY (Free OAuth — Nezaplacení kreditní karty)

| Model                 | Předpona | Limit         | Limit rychlosti |
| --------------------- | -------- | ------------- | --------------- |
| `Kimi-k2-neschopnost` | `if/`    | **Unlimited** | No reported cap |
| `qwen3-coder-plus`    | `if/`    | **Unlimited** | No reported cap |
| `hluboko-hledače-r1`  | `if/`    | **Unlimited** | No reported cap |
...
| `qwen3-kódér-flash` | `qw/`    | **Unlimited** | No reported cap     |
| `qwen3-kódér-next`  | `qw/`    | **Unlimited** | No reported cap     |
| `vizionářský model` | `qw/`    | **Unlimited** | Multimoda (obrázky) |
...
| Tier              | Denní limit          | Limit rychlosti | Poznámky                                                          |
| ----------------- | -------------------- | --------------- | ----------------------------------------------------------------- |
...
| Tier   | Denní limit       | Limit rychlosti  | Poznámky                                                   |
| ------ | ----------------- | ---------------- | ---------------------------------------------------------- |
| Zdarma | **1M tokenů/den** | 60K TPM / 30 RPM | Světově nejrychlejší inference LLM; každý den se restarted |
...
| `LongCat-Flash-Chat`          | `lc/`    | 500K tokens      | Multi-turn rozhovor           |
| `LongCat-Flash-Thinking`      | `lc/`    | 500K tokens      | Rozhodování / CoT             |
| `LongCat-Flash-Thinking-2601` | `lc/`    | 500K tokens      | Verze z ledna 2026            |
| `LongCat-Flash-Omni-2603`     | `lc/`    | 500K tokens      | Multimodální                  |
...
| `claude`           | `pol/`   | 1 req/15s       | Dodavatel Claude Anthropic |
| Dodavatel `gemini` | `pol/`   | 1 req/15s       | Google Gemini              |
| `deepseek`         | `pol/`   | 1 req/15s       | DeepSeek V3                |
| Dodavatel `llama`  | `pol/`   | 1 req/15s       | Skout Meta Llama 4         |
| `mistral`          | `pol/`   | 1 req/15s       | Dodavatel Mistral AI       |
...
| Tier   | Každodenní Neurony | Ekvivalentní použití                    | Poznámky                   |
| ------ | ------------------ | --------------------------------------- | -------------------------- |
...
| Tier   | Bezplatný limit | Místo        | Poznámky                                      |
| ------ | --------------- | ------------ | --------------------------------------------- |
...
| ---------------- | --------------------------------- | -------------------------------------------- | ---------------------------- |
| 🟢 **Deepgram**   | **200$ zdarma ** (registrovat se) | `nova-3` — nejlepší přesnost, 30+ jazyků     | No RPM limit on free credits |
| 🔵 **AssemblyAI** | **50$ zdarma ** (registrovat se)  | `universal-3-pro` — kapitoly, sentiment, PII | No RPM limit on free credits |
| 🔴 **Groq**       | **Zdarma věčně **                 | `whisper-large-v3` — OpenAI Whisper          | 30 RPM (omezený limitem)     |

**Doporučená kombinace v `/dashboard/combos`:**

```
Název: free-transcription
Strategie: Priorita
Nodes:
   [1] deepgram/nova-3           → pužije nejdříve volných 200 $
   [2] assemblyai/universal-3-pro → fallback when Deepgram credits run out
   [3] groq/whisper-large-v3    → free forever, emergency fallback
```

Potom v `/dashboard/media` → záložce **Transkripce**: nahrajte nějaký zvukový
nebo video soubor → vyberte svůj kombo endpoint → získejte transkripci v
...
| Feature                                        | What It Does                                                                                                                                                                                                                                                                                              |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🎮 **Model Playground**                         | Přístupová stránka ke testování jakéhokoli modelu přímo — poskytovatel/model/poUNUSED konektory, Monaco Editor, streamování, zrušení, časování                                                                                                                                                            |
| Porovnat CLI otisky                            | Nastavení pořadí hlaviček a těla na úrovni poskytovatele pro shodu s původní CLI podpisem — přepínač pro změnu v nastaveních > Bezpečnost. **Vaše proxy IP je uchováváno**                                                                                                                                |
...
| Přepis zvuku                                | [v1/audio/transcriptions`] — 7 poskytovatelů (Deepgram Nova 3, AssemblyAI, Groq Whisper, HuggingFace, ElevenLabs, OpenAI, Azure), automatické detekování jazyka, podpora formátů MP4/MP3/WAV |
...
| Porovnat CLI otisky                         | Souhlasí s jedinečnými signaturami příchozích požadavků od native CLI – **snížena riziko zablokování zatímco se udrží IP proxy serveru** |
...
| 🎮 **Model Playground**                          | Testujte jakýkoli poskytovatelmodel/ koncový bod z rozhraní            |
...
| Vlastní             | Základní funkce JavaScript vrátí true/false                       | `(výstup) =&gt; výstup.délka &gt; 10` |
...
```
Dashboard → Komba → Nové kombo

Název: premium-coding
Modely:
  1. cc/claude-opus-4-6 (Předplatné primárně)
  2. glm/glm-4.7 (Levná náhrada, 0,60 $/1 milion)
  3. minimax/MiniMax-M2.1 (Nejlevnější záloha, 0,20 $/1 milion)
